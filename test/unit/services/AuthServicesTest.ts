...
describe("AuthService", function () {
  ...
  describe("login", function () {
    it("should return token", async () => {
      const data = [
        "eyJhbGciOiJIUzI1NiJ9.eyJpYXQiOjE3MzYzNDEyOTAsImV4cCI6MTczNjM3MDA5MCwiZW1haWwiOiJUZXJyeUBnbWFpbC5jb20iLCJzdWIiOiJUZXJyeUBnbWFpbC5jb20iLCJpc3MiOiJLYWlub3MifQ.Dv1_YV2zS_I09SFQ-Ybach1V_6nLxJr3uLkEmF3TaHk",
      ];

      mock
        .onPost("http://localhost:8080/api/auth/login", loginRequest)
        .reply(200, data);
    ...
    it("should throw exception when 500 returned by axios", async () => {
      mock
        .onPost("http://localhost:8080/api/auth/login", badLoginRequest)
        .reply(500);
      try {
  ...
  describe("register", function () {
    it("should return token", async () => {
      ...
      mock
        .onPost("http://localhost:8080/api/auth/login", registerRequest)
        .reply(200, data);
      const results = await getToken(loginRequest);
